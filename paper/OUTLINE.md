...
> **Flag, resolved — see `01-hidden-preconditioner/FINDING_scale_attenuation.md`.**
> The Llama row's 6× is not robust. The clean replication (`llama_map`, a larger
> probe batch so the frames are constructed more accurately) gives **3.8×** with
> the floor at AdamW's own 2e-4 optimum — part of `llama_fp32`'s 6× was probe
> noise, not frame signal. Ordered by size the own-floor ratios read 0.6B 11×,
> 1B 10×, 3B 3.8×, 8B **washout** (0.11× the floor, n=5): the margin over the
> floor **attenuates with scale** (AdamW's reduction-order floor grows, the frame
> signal does not). This does not touch the §3 map, which is the *relative* gap
> AdamW ≫ blind; and the decisive check — the `llama_map` blind arm — is now in:
> at low lr muon, matprec and SGD collapse to their floor (0.00001, 0.00000,
> 0.00012) while AdamW and Lion hold 0.0009 and 0.0016, so the map holds on the
> third family. Framing is the user's call; the numbers are final.
